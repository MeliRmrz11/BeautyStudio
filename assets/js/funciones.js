                ...
                success: function (response) {
                    ...
                    if (res.message == 'OutOfStock') {
                        Swal.fire({
                            position: 'top-end',
                            icon: 'error',
                            title: 'Error, no hay suficiente stock de: ' + res.name,
                            showConfirmButton: false,
                            timer: 2000
                        })
                    } else if (response != 'error' && response != 'OutOfStock') {
                        Swal.fire({
                            position: 'top-end',
                            icon: 'success',
    ...
    $.ajax({
        ...
        success: function (response){
            response.forEach(row => {
                ...
                ${row['cantidad']}
                <button class="btn btn-success" type="button" onclick="incrementArti(${row['id']})">
                <i class="fas fa-solid fa-plus"></i>
                </button>
                <button class="btn btn-warning" type="button" onclick="decrementArti(${row['id']})">
                <i class="fas fa-sharp fa-solid fa-minus"></i>
                </button>
                </div></td>
...
function incrementArti(id){
    let detalle = 'incrementar'
    $.ajax({
        url: "ajax.php",
        data: {
            id: id,
        ...
        success: function (response) {
            ...
            if (response == 'aumentado') {
                ...
                Swal.fire({
                    position: 'top-end',
                    icon: 'error',
                    title: 'Error al aumentar el producto',
                    showConfirmButton: false,
                    timer: 2000
                })
            }
