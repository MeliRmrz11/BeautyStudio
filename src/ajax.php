...
if (isset($_GET['q'])) {
    ...
    if (!$datos) {
        $msg = "No existe producto a eliminar";
    }else{
    ...
    else if (isset($_GET['incrementar'])) {
        $id_detalle = $_GET['id'];
        $verificar = mysqli_query($conexion, "SELECT * FROM detalle_temp WHERE id = $id_detalle");
        $datos = mysqli_fetch_assoc($verificar);
            $cantidad = $datos['cantidad'] + 1;
            $query = mysqli_query($conexion, 
            "UPDATE detalle_temp SET cantidad = $cantidad WHERE id = $id_detalle");
            if ($query) {
                $msg = "aumentado";
            } else {
                $msg = "Error";
    ...
    if ($consulta) {
        ...
        while ($row = mysqli_fetch_assoc($consultaDetalle)) {
            ...
            $nameActual = mysqli_query($conexion, "SELECT descripcion FROM producto WHERE codproducto = $id_producto");
            $nameNuevo = mysqli_fetch_assoc($nameActual);
            $stockTotal = $stockNuevo['existencia'] - $cantidad;
        if($cantidad > $stockNuevo['existencia']){
            $msg = array('name' => $nameNuevo['descripcion'], 'message'=> 'OutOfStock');
